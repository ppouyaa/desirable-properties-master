...
def firstObjectiveForGroups(df, prices, costs, probFun):
    try:
        df_cpy = df.copy()
        df_indexes = list(df_cpy.index)
        df_list = [df_cpy.loc[idx] for idx in df_indexes]
        res = [
            firstObjective(df_temp, [price], [cost], probFun)
            for df_temp, price, cost in zip(df_list, prices, costs)
        ]
        return res
    except Exception as ex:
        raise Exception(str(ex))
...
def secondObjectiveForGroups(df, prices, probFun):
    try:
        df_cpy = df.copy()
        df_indexes = list(df_cpy.index)
        df_list = [df_cpy.loc[idx] for idx in df_indexes]
        res = [
            secondObjective(df_temp, [price], probFun)
            for df_temp, price in zip(df_list, prices)
        ]
        return res
    except Exception as ex:
        raise Exception(str(ex))
...
# ### Generating Random Probability of deal goes well values b/n 0.9 and 1
#

# In[12]:


def randomPDGW(size, prob=0.5):
    return [np.random.random() / 10 + prob for i in range(size)]
...
def thirdObjectiveOld(df, prices, probFun, PDGW, threshold, is_group=False):
    try:
        probs = probFun(df, prices)
        yds = [1 if prob > threshold else 0 for prob in probs]
        res = [yd * pdgw for yd, pdgw in zip(yds, PDGW)]
        if is_group:
            return res
        else:
            return np.sum(res)
...
def two_objectives(df, prices, costs, probFun):
    try:
        return [
...
def all_three_objectivesOld(df, prices, costs, probFun, PDGW, threshold):
    try:

        return [
            firstObjective(df, prices, costs, probFun),
            secondObjective(df, prices, probFun),
            thirdObjectiveOld(df, prices, probFun, PDGW, threshold),
        ]
    except Exception as ex:
        raise Exception(str(ex))
...
def pickCandidateBids(df, candidate_indexes):
    df_copy = df.copy()
    df_copy = df_copy[df_copy["Outcome"] == 1]
    candidate_list = []
    candidate_df = df_copy
...
def get_grouped_dfs(df, indexes):
    df_copy = df.copy()
    candidate_df = df_copy.loc[indexes]
    candidate_df.drop("Outcome", axis=1, inplace=True)
    return candidate_df


#  #### Probability of deals goes well value for differnt group sizes
#

# In[25]:


def assign_pdgw_values(PDGW, size):
    return [i % len(PDGW) for i in range(size)]


def cross_assign_pdgw_values(PDGW, start):
    pdgw_values = PDGW.copy()
    return pdgw_values[start:] + pdgw_values[:start]
...
# all_candidate_lists,candid_df,candid_indexes = pickCandidateBids(df,[3047])
...
# all_costs = randomCosts(df,candid_indexes)
# all_prices = getCandidatesOriginalPrices(df,candid_indexes)
...
# ### Optimize Achievement scalarizing function
#

# In[39]:


def Optimize_first_objective(df, function, probFun, costs, bound):
    try:
        f = lambda x: -1 * function(df, x, costs, probFun)
...
def OptimizeASF_two_objectives(df, function, probFun, costs, bound, reference, maxs):
    try:
        first_part = lambda x: [
            (function(df, x, costs, probFun)[i] - reference[i]) / maxs[i]
            for i in range(len(reference))
        ]
        second_part = lambda x: [
            (function(df, x, costs, probFun)[i]) / maxs[i]
            for i in range(len(reference))
        ]
        print(
            "First Part: {} Second Part: {}".format(
                first_part(costs[0]), second_part(costs[0])
            )
        )
        f = lambda x: -min(first_part(x)) - 0.001 * sum(second_part(x))
        x_opt = scipy.optimize.differential_evolution(f, bound, tol=TOL, atol=ATOL)
        return x_opt
    except Exception as ex:
        raise Exception(str(ex))
...
def OptimizeASF_three_objectives(
    df, function, probFun, costs, PDGW, bound, reference, maxs, threshold
):
    try:
        f = lambda x: -min(
            [
                (function(df, x, costs, probFun, PDGW, threshold)[i] - reference[i])
                / maxs[i]
                for i in range(len(reference))
            ]
        )
        -0.001 * sum(
            [
                (function(df, x, costs, probFun, PDGW, threshold)[i]) / maxs[i]
                for i in range(len(reference))
            ]
        )
        # if sum(costs)<=sum(x) else 1e10 #if any([xi <= upper_bound for xi in x]) else 1e10
        x_opt = scipy.optimize.differential_evolution(f, bound, tol=TOL, atol=ATOL)
        return x_opt
    except Exception as ex:
        raise Exception(str(ex))
...
def OptimizeASF_two_objectivesModified(
    df, function, probFun, costs, bound, reference, Ideal, Nadir
):
    try:
        first_part = lambda x: [
            (function(df, [x], costs, probFun)[i] - reference[i])
            / (Nadir[i][i] - Ideal[i][i])
            for i in range(len(reference))
        ]
        second_part = lambda x: [
            (function(df, [x], costs, probFun)[i]) / (Nadir[i][i] - Ideal[i][i])
            for i in range(len(reference))
        ]
        f = lambda x: -min(first_part(x)) - 0.000001 * sum(second_part(x))
        x_opt = scipy.optimize.differential_evolution(f, bound, tol=TOL, atol=ATOL)
        return x_opt
    except Exception as ex:
        raise Exception(str(ex))
                ...
                for k, pdgw in enumerate(PDGW):
                    ...
                    try:
                        IDEAL_VECTOR, NADIR_VECTOR = getIdealAndNadir(
                            i, deal, thirdObjectiveConstDec, 0.5, X[0], Y[0]
                        )
...
# ### Three objectives, third objective exponentially decreasing
#

# ### Exponential third objective function
#
